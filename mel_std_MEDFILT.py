...
# -*- coding: utf-8 -*-
"""
Created on Wed May  6 13:22:33 2020
...
for root, dirs, files in os.walk('/data/stor/basic_data/tri_data/rink/proc_data/'):
    for name in sorted(files):
        if name.endswith(('.MEDFILT.unw.rec')):
            file_paths= os.path.join(root, name)
            flist.append(file_paths) 
            allfiles= np.array(flist)


            if file_paths.startswith(('/data/stor/basic_data/tri_data/rink/proc_data/d0728/INT/MED/ave30min/')):
                with open(file_paths,'rb') as f:
                        vel_= np.fromfile(file_paths, dtype='>f')
                        vel_[vel_==0] = np.nan
                        vel_rectangle= np.reshape(vel_, (1559,845))#[600:630,755:785]
                        mel0 = vel_rectangle# LOS melange velocities
                        mel_1.append(mel0[694, 220])
                        mel_2.append(mel0[802, 155]) 
                        t_start0.append(dt.datetime.strptime(name[:15], '%Y%m%d_%H%M%S'))
                        t_end0.append(dt.datetime.strptime(name[17:32], '%Y%m%d_%H%M%S'))  
                        
            if file_paths.startswith(('/data/stor/basic_data/tri_data/rink/proc_data/d0729/MED/')):
                with open(file_paths,'rb') as f:
                        vel_= np.fromfile(file_paths, dtype='>f')
                        vel_[vel_==0] = np.nan
                        vel_rectangle= np.reshape(vel_, (1559,845))#[600:630,755:785]
            ...
            if file_paths.startswith(('/data/stor/basic_data/tri_data/rink/proc_data/d0731/MED/')):
                with open(file_paths,'rb') as f:
                        vel_= np.fromfile(file_paths, dtype='>f')
                        vel_[vel_==0] = np.nan
                        vel_rectangle= np.reshape(vel_, (1559,845))#[600:630,755:785]
            ...
            if file_paths.startswith(('/data/stor/basic_data/tri_data/rink/proc_data/d0802/MED/')):
                with open(file_paths,'rb') as f:
                        vel_= np.fromfile(file_paths, dtype='>f')
                        vel_[vel_==0] = np.nan
                        vel_rectangle= np.reshape(vel_, (1559,845))#[600:630,755:785]
...
plt.imshow(mel0)
plt.plot(220, 694, 'yo', markersize = 2)
plt.plot(155, 802, 'yo', markersize = 2)
                #%%
mli_size = (1248, 677)
...
axes[0,0].plot(t_start0, mel_1, '.', color='goldenrod', markersize = 1)

axes[0,0].plot(t_start0, mel_2,'.', color='blue', markersize = 1)
