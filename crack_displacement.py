...
displacement0 = np.array([sum(ice0[:i]) for i in range(len(ice0))])*time_between_samples  #green open
displacement0a = np.array([sum(ice0a[:i]) for i in range(len(ice0a))])*time_between_samples  #green closed   
...
displacement1 = np.array([sum(ice1[:i]) for i in range(len(ice1))])*time_between_samples     
displacement1a = np.array([sum(ice1a[:i]) for i in range(len(ice1a))])*time_between_samples
...
trend = np.polyval(coeff0, t_range)
# note that simply subtracting the trend might not be enough for other data sets
plt.plot(t_start, displacement0 - trend, '--', color = 'limegreen', linewidth = 2)
plt.plot(t_start, displacement0a - trend, '-', color = 'limegreen', linewidth = 2)
fig.show()
